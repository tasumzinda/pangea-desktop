...
import com.itech.pangea.business.domain.PMTCTFOForm;
import com.itech.pangea.business.repo.PMTCTFOFormRepo;
import com.itech.pangea.business.service.PMTCTFOFormService;
...
public class PMTCTFOFormServiceImpl implements PMTCTFOFormService {
 
    @Resource
    private PMTCTFOFormRepo pMTCTFOFormRepo;
    @PersistenceContext
    private EntityManager entityManager;
    
    @Override
    public List<PMTCTFOForm> getAll(){
        return pMTCTFOFormRepo.findAll();
    }
    
    @Override
    @Transactional
    public PMTCTFOForm save(PMTCTFOForm t) {
        if (t.getId() == null) {
            t.setDateCreated(new Date());
            return pMTCTFOFormRepo.save(t);
        }
    ...
    public Long get(SearchNationalDTO dto, String colName) {
        StringBuilder builder = new StringBuilder("Select sum(p."+colName+") from PMTCTFOForm p");
        int position = 0;
