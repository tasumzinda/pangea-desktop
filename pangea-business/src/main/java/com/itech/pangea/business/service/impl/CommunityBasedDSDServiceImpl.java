...
import com.itech.pangea.business.domain.CommunityBasedDSD;
import com.itech.pangea.business.repo.CommunityBasedDSDRepo;
import com.itech.pangea.business.service.CommunityBasedDSDService;
...
public class CommunityBasedDSDServiceImpl implements CommunityBasedDSDService{
    
    @Resource
    private CommunityBasedDSDRepo repo;
    @Resource
    ...
    @Override
    public List<CommunityBasedDSD> getAll() {
        return repo.findByActive(Boolean.TRUE);
    }
    @Override
    @Transactional
    public CommunityBasedDSD save(CommunityBasedDSD t) {
        if (t.getId() == null) {
    ...
    public Long get(SearchNationalDTO dto, String colName) {
        StringBuilder builder = new StringBuilder("Select sum(p."+colName+") from CommunityBasedDSD p");
        int position = 0;
