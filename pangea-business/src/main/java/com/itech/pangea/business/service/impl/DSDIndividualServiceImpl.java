...
import com.itech.pangea.business.domain.DSDIndividual;
import com.itech.pangea.business.repo.DSDIndividualRepo;
import com.itech.pangea.business.service.DSDIndividualService;
...
public class DSDIndividualServiceImpl implements DSDIndividualService {

    @Resource
    private DSDIndividualRepo dsdIndividualRepo;
    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public List<DSDIndividual> getAll() {
        return dsdIndividualRepo.findAll();
    }

    @Override
    @Transactional
    public DSDIndividual save(DSDIndividual t) {
        if (t.getId() == null) {
            t.setDateCreated(new Date());
            return dsdIndividualRepo.save(t);
        }
    ...
    public Long get(SearchNationalDTO dto, String colName) {
        StringBuilder builder = new StringBuilder("Select sum(p."+colName+") from DSDIndividual p");
        int position = 0;
