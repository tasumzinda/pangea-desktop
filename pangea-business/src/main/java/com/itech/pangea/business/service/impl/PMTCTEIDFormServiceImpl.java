...
import com.itech.pangea.business.domain.PMTCTEIDForm;
import com.itech.pangea.business.domain.Period;
import com.itech.pangea.business.repo.PMTCTEIDFormRepo;
import com.itech.pangea.business.service.PMTCTEIDFormService;
...
public class PMTCTEIDFormServiceImpl implements PMTCTEIDFormService{
    
    
    @Resource
    private PMTCTEIDFormRepo repo;
    @Resource
    ...
    @Override
    public List<PMTCTEIDForm> getAll() {
        return repo.findByActive(Boolean.TRUE);
    }

    @Override
    public PMTCTEIDForm get(Long id) {
        if (id == null) {
    ...
    @Override
    public void delete(PMTCTEIDForm t) {
        if (t.getId() == null) {
    ...
    @Override
    public List<PMTCTEIDForm> getPageable() {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    @Transactional
    public PMTCTEIDForm save(PMTCTEIDForm t) {
        if (t.getId() == null) {
    ...
    @Override
    public Boolean checkDuplicate(PMTCTEIDForm current, PMTCTEIDForm old) {
        throw new UnsupportedOperationException("implement method");
    }

    @Override
    public List<PMTCTEIDForm> getByPeriod(Period period) {
        return repo.findByPeriodAndActive(period, Boolean.TRUE);
    }

    @Override
    public Long get(SearchNationalDTO dto, String colName) {
        StringBuilder builder = new StringBuilder("Select sum(p."+colName+") from PMTCTEIDForm p");
        int position = 0;
