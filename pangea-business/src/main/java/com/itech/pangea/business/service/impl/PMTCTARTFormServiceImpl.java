...
import com.itech.pangea.business.domain.PMTCTARTForm;
import com.itech.pangea.business.repo.PMTCTARTFormRepo;
import com.itech.pangea.business.service.PMTCTARTFormService;
...
public class PMTCTARTFormServiceImpl implements PMTCTARTFormService {
    
    @Resource
    private PMTCTARTFormRepo pMTCTARTFormRepo;
    @PersistenceContext
    private EntityManager entityManager;
    
    @Override
    public List<PMTCTARTForm> getAll(){
        return pMTCTARTFormRepo.findAll();
    }
    
    @Override
    @Transactional
    public PMTCTARTForm save(PMTCTARTForm t) {
        if (t.getId() == null) {
            t.setDateCreated(new Date());
            return pMTCTARTFormRepo.save(t);
        }
    ...
    public Long get(SearchNationalDTO dto, String colName) {
        StringBuilder builder = new StringBuilder("Select sum(p."+colName+") from PMTCTARTForm p");
        int position = 0;
