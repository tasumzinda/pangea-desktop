 ...
 * @author user
 */
public class DefaulterTrackingRepo {
       public  SqliteDatabaseHandler handle;

        public DefaulterTrackingRepo() {
        ...
        public  Integer findLastInsertByUserAndFacility(Integer userId, Integer facilityId) throws SQLException{
                    String query = "Select max(id) from defaulter_tracking_form where created_by='"+ userId + "' and facility='" + facilityId +"'";
                ...
                public  String getSequentialNumber(Integer formId) throws SQLException{
                    String query = "Select sequential_number from defaulter_tracking_form where id='"+ formId +"'";
                    ResultSet rs  = handle.execQuery(query);
                    String number = "";
                    while(rs.next()){
                ...
                public  String getDateCreated(Integer formId) throws SQLException{
                    String query = "Select date_created from defaulter_tracking_form where id='"+ formId +"'";
                    ResultSet rs  = handle.execQuery(query);
                    String number = "";
                    while(rs.next()){
                        number = rs.getString(1);
